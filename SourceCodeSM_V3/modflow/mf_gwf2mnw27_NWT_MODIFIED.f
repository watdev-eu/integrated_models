...
! MODIFICATION Chnage name to 2
      TYPE GWFMNWTYPE2
        INTEGER,POINTER  ::NMNW2V2,MNWMAXV2,NMNWVLV2,IWL2CBV2,MNWPRNTV2
        INTEGER,POINTER  ::NODTOTV2,INTTOTV2,NTOTNODV2
        DOUBLE PRECISION, POINTER :: SMALLV2
        CHARACTER(LEN=20), DIMENSION(:),   POINTER     ::WELLIDV2
        CHARACTER(LEN=16), DIMENSION(:),   POINTER     ::MNWAUXV2
        DOUBLE PRECISION,  DIMENSION(:,:), POINTER     ::MNW2V2
        DOUBLE PRECISION,  DIMENSION(:,:), POINTER     ::MNWNODV2
        DOUBLE PRECISION,  DIMENSION(:,:), POINTER     ::MNWINTV2
        DOUBLE PRECISION,  DIMENSION(:,:,:), POINTER     ::CapTableV2
      END TYPE
      TYPE(GWFMNWTYPE2), SAVE:: GWFMNWDAT2(10)
...
C2------IDENTIFY PACKAGE AND INITIALIZE NMNW2.
      WRITE(IOUT,1)IN
    1 format(/,1x,'MNW2 -- MULTI-NODE WELL 2 PACKAGE, VERSION 1.0.5,',
     +' 04/04/2012.',/,4X,'INPUT READ FROM UNIT ',i3)
      NMNW2=0
      ntotnod=0
C
...
C     ------------------------------------------------------------------
     ...
     & hwell,verysmall,hlim,qpot,cond,ratio,hmax_NEW,hsim, ! MODIFICATION renamed to_NEW
     & QCUT,qnet,hhnew
C
      CALL SGWF2MNW2PNT(IGRID)
C
...
C     ------------------------------------------------------------------
      ...
      INTEGER PUMPCAP,firstnode,lastnode, Iuupw,iqslv
      DOUBLE PRECISION verysmall,qdes,hlim,hwell,qact,
     & cond,dhc2w,hmax_NEW,hsim,ratio,qactCap,Hlift, ! MODIFICATION Renamed to _NEW
...
c   If iqslv=1, Well will be simulated as a GHB 
                  iqslv = 0
                  if( hsim.gt.hmax_NEW .and. hmax_NEW.gt.verysmall )THEN 
			iqslv = 1 
		  END IF
                  if((qdes-qact)**2 .gt. small               )THEN 
			iqslv = 1 
		  END IF
                  if(abs(qact).lt.verysmall .and.hsim.gt.hmax_NEW)THEN
			 iqslv = 0 
	          END IF
                  if(abs(qact).lt.verysmall .and.hsim.lt.hmax_NEW)THEN
			 iqslv = 1 
		 END IF
...
c  check to write message that inode was deactivated this time step
  ...
  200 FORMAT ('Node no. ',I3,'  of Multi-Node Well ', A20)
...
c
     ...
     & qnet,' because of Pump Capacity restraint'
                else if (mnw2(6,iw).NE.0) then
                  write(iout,*) WELLID(iw),' Qdes has been updated to',
     & qnet,' because of Hlim constraint'
                else
                  write(iout,*) WELLID(iw),' Qnet = ',qnet,' < Qdes'
                end if
            end if
...
C     ------------------------------------------------------------------
     ...
     & hwell,hlim,verysmall,hmax_NEW,hsim,bottom,qcut,qoff,qon,qsmall, ! MODIFICATION renamed to _NEW
     & qpot,cond,ratio
C
      verysmall = 1.D-25
...
C  Change MNW data to a different grid.
      USE GWFMNW2MODULE
C
!MODIFICATION Change to 2
        NMNW2=>GWFMNWDAT2(IGRID)%NMNW2V2
        MNWMAX=>GWFMNWDAT2(IGRID)%MNWMAXV2
        NMNWVL=>GWFMNWDAT2(IGRID)%NMNWVLV2
        IWL2CB=>GWFMNWDAT2(IGRID)%IWL2CBV2
        MNWPRNT=>GWFMNWDAT2(IGRID)%MNWPRNTV2
        NODTOT=>GWFMNWDAT2(IGRID)%NODTOTV2
        INTTOT=>GWFMNWDAT2(IGRID)%INTTOTV2
        NTOTNOD=>GWFMNWDAT2(IGRID)%NTOTNODV2
        SMALL=>GWFMNWDAT2(IGRID)%SMALLV2
        WELLID=>GWFMNWDAT2(IGRID)%WELLIDV2
        MNWAUX=>GWFMNWDAT2(IGRID)%MNWAUXV2
        MNW2=>GWFMNWDAT2(IGRID)%MNW2V2
        MNWNOD=>GWFMNWDAT2(IGRID)%MNWNODV2
        MNWINT=>GWFMNWDAT2(IGRID)%MNWINTV2
        CapTable=>GWFMNWDAT2(IGRID)%CapTableV2
C
      RETURN
      END
...
C
!MODIFICATION Change to 2
        GWFMNWDAT2(IGRID)%NMNW2V2=>NMNW2
        GWFMNWDAT2(IGRID)%MNWMAXV2=>MNWMAX
        GWFMNWDAT2(IGRID)%NMNWVLV2=>NMNWVL
        GWFMNWDAT2(IGRID)%IWL2CBV2=>IWL2CB
        GWFMNWDAT2(IGRID)%MNWPRNTV2=>MNWPRNT
        GWFMNWDAT2(IGRID)%NODTOTV2=>NODTOT
        GWFMNWDAT2(IGRID)%INTTOTV2=>INTTOT
        GWFMNWDAT2(IGRID)%NTOTNODV2=>NTOTNOD
        GWFMNWDAT2(IGRID)%SMALLV2=>SMALL
        GWFMNWDAT2(IGRID)%WELLIDV2=>WELLID
        GWFMNWDAT2(IGRID)%MNWAUXV2=>MNWAUX
        GWFMNWDAT2(IGRID)%MNW2V2=>MNW2
        GWFMNWDAT2(IGRID)%MNWNODV2=>MNWNOD
        GWFMNWDAT2(IGRID)%MNWINTV2=>MNWINT
        GWFMNWDAT2(IGRID)%CapTableV2=>CapTable
C
      RETURN
      END

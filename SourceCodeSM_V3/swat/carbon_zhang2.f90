      ...
      !!===========================================
       do k = 1, sol_nly(j)
          ...
          if (sol_tmp(k,j) > 0. .AND. sol_st(k,j) > 0.) then
		      ...
		      upper=0.8*(sol_z(kk,j)-sol_z(kk-1,j))/2.0
		      lower=(sol_z(kk,j)+sol_z(kk-1,j))/2.0+EXP(18.40961-0.023683632*(sol_z(kk,j)+sol_z(kk-1,j))/2.0) 	  
		      !OX=1.0-0.8*((sol_z(kk,j)+sol_z(kk-1,j))/2)/(((sol_z(kk,j)+sol_z(kk-1,j))/2)

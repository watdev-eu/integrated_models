...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      use parm
      use smrt_parm, only:mf_active !SWAT-MODFLOW
	 ...
	 normOut = .false.
      ...
      !! HRU summations
        ...
        if(normOut) hrumono(6,j) = hrumono(6,j) + gw_q(j)  !tcw
        ...
        if(normOut) hrumono(70,j) = hrumono(70,j) + gw_qdeep(j) !tcw
      ...
      !! watershed summations
        ...
        !rtb (02-01-2017)
        !only perform following sum if SWAT-MODFLOW is inactive. If SWAT-MODFLOW
        !is active, then MODFLOW and RT3D will provide these values.
        ...
        !rtb modflow - this is performed in "smrt_conversion2mf" when SWAT-MODFLOW is active
        if(mf_active.eq.0) then
        ...
        do ii=1,mstdo
          ...
          if(normOut) fcstaao(8) = fcstaao(8) + gw_q(j) * hru_dafr(j) !tcw

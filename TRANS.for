...
!  Calls:     None
C=======================================================================
      SUBROUTINE TRANS(CONTROL,DYNAMIC, MEEVP,
...
!-----------------------------------------------------------------------
	...
	REAL CHT,CANHT,KCBMIN,KCBMAX,APU,SKC,ET_ALB,SRAD,TMAX,WINDHT
	REAL WINDRUN,XELEV,XLAT
	INTEGER DOY,YEAR,YRDOY
	TYPE(Plant_GROWTH) PlantT
...
!                    CALL PETASCE(
!     &        CANHT, DOY, ET_ALB, 'S', SRAD, TDEW,      !Input 
...
!     &        EO)                                         !Output
	...
	CHT = MAX(0.001,CANHT)
        KCB = 0.0
	APU=KCBMIN+(KCBMAX-KCBMIN)*(1.0-EXP(-1.0*SKC*XHLAI))*(CHT/3.0)**0.3
        KCB = MAX(0.0,APU,KCB+0.05)
	!KCB=MAX(KCB,0.0)
	CALL GET(PlantT)
	!CNHT=PlantT%CANHT

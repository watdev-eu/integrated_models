...
!!!! Srin's irrigation source by each application changes
	...
	write(*,*)'sol_sumfc=',sol_sumfc(j)
     ...
     & (wstrs_id(j)==2.and.sol_sumfc(j)-sol_sw(j)>auto_wstr(j)))) then  
        ...
        vmmd = 0.
        vmm = 0.
	!write(*,*)'Autoirr:irrsc=',irrsc(j)
        select case (irrsc(j))
          case (3)   !! shallow aquifer source
            if(mf_active.eq.0) then !rtb modflow
              do k = 1, nhru
              ...
              vmm = sol_sumfc(j) !initially set equal to field capacity
                                 !(can be modified in code below)
            endif
	...
	!write(*,*)'vmm=',vmm
      !! if water available from source, proceed with irrigation
        ...
        !swat-modflow auto irrigation
        if(mf_active.eq.1) then
          
          !set irrigation depth to the depth specified in the .mgt file

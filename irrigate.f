...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
	...
	USe ModuleData
      ...
      integer nn,cell_row,cell_col,cell_lay !rtb modflow
...
!! the variable is initialized here)
      ...
      if(mf_active.eq.1 .and. mf_irrigation_swat.eq.1) then
        ...
        do nn=1,ncell_irrigate
          if(jj.eq.mf_sub_irrigate(nn,4)) then !found HRU
            
            !row and column of the WELL cell
            cell_row = mf_sub_irrigate(nn,2)
            cell_col = mf_sub_irrigate(nn,3)
            cell_lay = mf_sub_irrigate(nn,5)
...
!	write(*,*)'aird=',aird(jj),'volmm=',volmm,'sq_rto=',sq_rto

	if(aird(jj)>0)STOP
	CALL PUT('MGMT','IRRAMT',aird(jj))
	CALL GET('MGMT','IRRAMT',APU)	
	!write(*,*)'IRRIGATION APU=',APU
	!IF(aird(jj)>0)STOP	

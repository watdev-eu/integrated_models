...
C              ERROR
C=======================================================================
      SUBROUTINE GROW_DSSAT(CONTROL, ISWITCH, DYNAMIC, SOILPROP, 
...
!-----------------------------------------------------------------------
	...
	USE ModuleData ! MODIFICATION JJ 
	USE interface2
      IMPLICIT NONE
      SAVE
!-----------------------------------------------------------------------
...
!	write(*,*)'GROW,RUNINIT'
	...
	PlantG3%PROLFF=PROLFF !
	PlantG3%PROSTF=PROSTF !
	PlantG3%PRORTF=PRORTF !	
	PlantG3%PROSHF=PROSHF ! 
	PlantG3%PRONOD=PRONOD ! 
	PlantG3%PROLFI=PROLFI ! 
	PlantG3%PROSTI=PROSTI !
	PlantG3%PRORTI=PRORTI ! 
	PlantG3%PLTPOP=PLTPOP !
	PlantG3%ROWSPC=ROWSPC ! 
	PlantG3%RMIN=RMIN !   	
	PlantG3%PLME=PLME !
	PlantG3%SDWTPL=SDWTPL ! 
	PlantG3%SDLIP=SDLIP !
	PlantG3%SDPRO=SDPRO !
	PlantG3%WTFSD=WTFSD !
	PlantG3%WTPSD=WTPSD ! 
	...
	PlantG3%ALPHR=ALPHR !
	PlantG3%ALPHS=ALPHS
	PlantG3%ALPHSH=ALPHSH!
	PlantG3%PCARLF=PCARLF!
	PlantG3%PCARST=PCARST! 
	PlantG3%PCARSH=PCARSH!
	PlantG3%PCARSD=PCARSD!
	PlantG3%PCARNO=PCARNO!
	PlantG3%PLIGLF=PLIGLF!
	PlantG3%PLIGST=PLIGST!
	PlantG3%PLIGRT=PLIGRT!
	PlantG3%PLIGSH=PLIGSH!
	PlantG3%PLIGSD=PLIGSD!
	PlantG3%PLIGNO=PLIGNO!
	PlantG3%PLIPLF=PLIPLF!
	PlantG3%PLIPST=PLIPST!
	PlantG3%PLIPRT=PLIPRT!
	PlantG3%PLIPSH=PLIPSH!
	PlantG3%PLIPNO=PLIPNO!
	PlantG3%PMINLF=PMINLF!
	PlantG3%PMINST=PMINST!
	PlantG3%PMINRT=PMINRT!
	PlantG3%PMINSH=PMINSH!
	PlantG3%PMINSD=PMINSD!
	PlantG3%PMINNO=PMINNO!
	PlantG3%POALF=POALF ! 
	PlantG3%POAST=POAST !
	PlantG3%POART=POART !
	PlantG3%POASH=POASH!
...
!-----------------------------------------------------------------------
	PlantG3%CPFLF=CPFLF !
	PlantG3%CPFSTM=CPFSTM!
	PlantG3%CPFRT=CPFRT!
	PlantG3%CPFNOD=CPFNOD!
	PlantG3%CPFSH1=CPFSH1!
	PlantG3%CPFSD1=CPFSD1!
	PlantG3%PCNMIN=PCNMIN
	CALL PUT(PlantG3)
      ENDIF
...
!	PlantG%WTFL=WTFL
	...
	PlantG3%WTSHO=WTSHO
	PlantG3%WTSDO=WTSDO
!	PlantG%WTNDOO=WTNDOO
	PlantG3%WTCO=WTCO
	PlantG3%NLPEST=NLPEST
	PlantG3%WTNLF=WTNLF
	PlantG3%WTNST=WTNST
	PlantG3%WTNRT=WTNRT
	PlantG3%WTNSH=WTNSH
	PlantG3%WTNSD=WTNSD
	PlantG3%WTNCAN=WTNCAN
	PlantG3%WTNSA=WTNSA
	PlantG3%WTNRA=WTNRA
	PlantG3%WTNSHA=WTNSHA
	PlantG3%WTNSA=WTNSA
	PlantG3%WTNRA=WTNRA
	PlantG3%WTNSHA=WTNSHA
	PlantG3%WTNSDA=WTNSDA
	PlantG3%DWNODA=DWNODA
	PlantG3%WTNFX=WTNFX
	PlantG3%WTNLO=WTNLO
	PlantG3%WTNSO=WTNSO
	PlantG3%WTNRO=WTNRO
	PlantG3%WTNSHO=WTNSHO
	PlantG3%WTNSDO=WTNSDO
	PlantG3%WTNNO=WTNNO
	PlantG3%WTNMOB=WTNMOB
	PlantG3%WTNUP=WTNUP
	PlantG3%WTNNA=WTNNA
	PlantG3%PUNCSD=PUNCSD
	PlantG3%SEEDNO=SEEDNO
	PlantG3%PUNCTR=PUNCTR
	PlantG3%WSHDOT=WSHDOT
	!PlantG%PLTPOP=PLTPOP
	CALL PUT(PlantG3)
...
!-----------------------------------------------------------------------
	...
	ALPHS=PlantG3%ALPHS
	ALPHR=PlantG3%ALPHR
	ALPHSH=PlantG3%ALPHSH
	PROLFI=PlantG3%PROLFI
	PROSTI=PlantG3%PROSTI
	PRORTI=PlantG3%PRORTI
!     Net growth rates
      WLDOT  = 0.0
      WSDOT  = 0.0
      WRDOT  = 0.0
	...
	PlantG3%WNDOT=WNDOT
	PlantG3%WPDOT=WPDOT
	PlantG3%WLFDOT=WLFDOT

	PLME=PlantG3%PLME
	SDWTPL=PlantG3%SDWTPL
...
!	PlantG%WTNDOO=WTNDOO
      ...
      WTNSH=PlantG3%WTNSH  !
	  WTNSD=PlantG3%WTNSD!
	  WTNCAN=PlantG3%WTNCAN !
      WTNSA=PlantG3%WTNSA !
	  !WTNRA=PlantG3%WTNRA
	  WTNSHA=PlantG3%WTNSHA
	  WTNSA=PlantG3%WTNSA
!WTNRA=PlantG3%WTNRA
	 
!WTNSHA=PlantG3%WTNSHA
      WTNSDA=PlantG3%WTNSDA
	  DWNODA=PlantG3%DWNODA
	  WTNFX=PlantG3%WTNFX
	  WTNLO=PlantG3%WTNLO
	  WTNSO=PlantG3%WTNSO
	  WTNRO=PlantG3%WTNRO
	  WTNSHO=PlantG3%WTNSHO
	  WTNSDO=PlantG3%WTNSDO
      WTNNO=PlantG3%WTNNO
	  WTNMOB=PlantG3%WTNMOB
	  WTNUP=PlantG3%WTNUP
	  WTNNA=PlantG3%WTNNA
	  PUNCSD=PlantG3%PUNCSD
	  SEEDNO=PlantG3%SEEDNO
	  PUNCTR=PlantG3%PUNCTR
	  PLTPOP=PlantG3%PLTPOP
	  STMWT=PlantG3%STMWT	
	CPFLF=PlantG3%CPFLF
	CPFSTM=PlantG3%CPFSTM
	CPFSD1=PlantG3%CPFSD1
	CPFRT=PlantG3%CPFRT
	CPFNOD=PlantG3%CPFNOD
	CPFSH1=PlantG3%CPFSH1	
	WSHDOT=PlantG3%WSHDOT
	ALPHL=PlantG3%ALPHL
	ALPHR=PlantG3%ALPHR
	ALPHS=PlantG3%ALPHS
	ALPHSH=PlantG3%ALPHSH
...
C-----------------------------------------------------------------------
	...
	!WRITE(*,*)'WLDOT=',WLDOT,'WLDOTN=',WLDOTN,'SLDOT=',SLDOT
	!write(*,*)'WLIDOT=',WLIDOT,'WLFDOT=',WLFDOT,'NRUSLF=',NRUSLF
	!write(*,*)'CRUSFL=',CRUSLF
	!write(*,*)'WGWGWGGWGWGWGWGWGWGWGWGWGWGWGWGW'
...
!     A positive value represents leaf mass lost. (kg/ha)
	...
	WTLF=PlantG3%WTLF	
     ...
     &    (1. - MIN(1.0,(SLDOT+WLIDOT+WLFDOT)/WTLF))
      ...
      ENDIF
      !write(*,*)'N and C mobilization ShutMob=',ShutMob
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
	...
	!write(*,*)'WSDOT=',WSDOT
	!Write(*,*)'WSDOTN=',WSDOTN,'SSDOT=',SSDOT,'WSIDOT=',WSIDOT
	!write(*,*)'NRUSST=',NRUSST,'CRUSST=',CRUSST

!	write(*,*)'STMWT=',STMWT
      IF (STMWT > 1.E-4) THEN
         WSDOT = WSDOT + (CADST+NADST/0.16) *
     &   (1. - MIN(1.0,(SSDOT+WSIDOT)/STMWT))
	!write(*,*)'CADST=',CADST,'NADST=',NADST
	!write(*,*)'SSDOT=',SSDOT,'WSIDOT=',WSIDOT
...
C-----------------------------------------------------------------------
	...
	!write(*,*)'WRCSDT=',WRCSDT
	!write(*,*)'ALPHS=',ALPHS,'WSDOT=',WSDOT
	!write(*,*)'CRUSST=',CRUSST,'RHOS=',RHOS,'SSNDOT=',SSNDOT
      IF (STMWT > 1.E-4) THEN
         WRCSDT = WRCSDT + CADST *
     &     (1. - MIN(1.0,(SSDOT+WSIDOT)/STMWT))
      ENDIF
      ...
      !write(*,*)'WRCSHD=',WRCSHD,'ALPHSH=',ALPHSH,'WSHDOT=',WSHDOT
	!write(*,*)'CRUSSH=',CRUSSH,'RHOSH=',RHOSH
	!write(*,*)'WTABRT=',WTABRT,'WTSHMT=',WTSHMT,'WSHIDT=',WSHIDT	
C-----------------------------------------------------------------------
C     Update C Storage, Concentrations in Leaves, Stems, Roots, and Shells
C-----------------------------------------------------------------------
	!write(*,*)'WCRSH=',WCRSH,'WRCSHD=',WRCSHD	
	WCRLF=PlantG3%WCRLF
	WCRST=PlantG3%WCRST
	WCRRT=PlantG3%WCRRT
	WCRSH=PlantG3%WCRSH	
...
!	write(*,*)'WCRSH=',WCRSH
!	Write(*,*)'...............C storage.......................'	
      IF (WCRLF .LE. 1.0E-30)THEN
	 WCRLF = 0.0
      END IF
      IF (WCRST .LE. 1.0E-30)THEN
	 WCRST = 0.0 
      END IF
      IF (WCRRT .LE. 1.0E-30)THEN
	 WCRRT = 0.0
      END IF
      IF (WCRSH .LE. 1.0E-30)THEN
	 WCRSH = 0.0
      END IF	

	!write(*,*)'GROW_DSSAT,INTEGROINTI''WCRST=',WCRST,'WRCSDT=',WRCSDT
	!write(*,*)'OOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO'
	PlantG3%WCRST=WCRST	
	PlantG3%WCRLF=WCRLF
	PlantG3%WCRRT=WCRRT
	PlantG3%WCRSH=WCRSH
...
!	write(*,*)'WTNLF=',WTNLF
      WTNLF = WTNLF + NLDOT
	PlantG3%WTNLF=WTNLF
!	write(*,*)'WTNLF=',WTNLF,interface_ihru
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
      ...
      ELSE
        WNRST = 0.0
      ENDIF
	PRORTF=PlantG3%PRORTF
      IF (RTWT > 1.E-4) THEN
        WNRRT = MAX (WTNRT - PRORTF * 0.16 * RTWT, 0.0)
      ELSE
        WNRRT = 0.0
      ENDIF
	PROSHF=PlantG3%PROSHF
...
C       plant matter
C-----------------------------------------------------------------------
	PLIGLF=PlantG3%PLIGLF
	PLIGST=PlantG3%PLIGST
	PLIGSH=PlantG3%PLIGSH
...
C-----------------------------------------------------------------------
	PLIGRT=PlantG3%PLIGRT
	PLIGNO=PlantG3%PLIGNO
      DO L = 1, NLAYR 
...
C-----------------------------------------------------------------------
	...
	!write(*,*)'WLDOTN=',WLDOTN,'F=',F,'SLDOT=',SLDOT
	!write(*,*)'WLIDOT=',WLIDOT,'NRUSLF=',NRUSLF,'SLA=',SLA
	!write(*,*)'WLFDOT=',WLFDOT
...
!	PlantG%WTNDOO=WTNDOO
		...
		PlantG3%PLTPOP=PLTPOP
		CALL PUT(PlantG3)
		!write(*,*)'PlantG3%TOTWT=',PlantG3%TOTWT
		!write(*,*)'PlantG3%TOPWT=',PlantG3%TOPWT
		!write(*,*)'WTNTOT=',WTNTOT,PlantG3%WTNTOT
		!write(*,*)'ZZZZZZZZZZZZZZZZZZZZZZZZZZ-END--------------'
...
!      CHARACTER*255 C255
	...
	IF(useSWAT)THEN
	CALL interface_IPGROW(CROP,PLTPOP,PLME,ROWSPC,SDWTPL,ECONO,WTPSD,
     &SDPRO,SDLIP)
	ELSE
!-----------------------------------------------------------------------

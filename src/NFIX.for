...
!-----------------------------------------------------------------------
	IF(useSWAT)THEN
	CALL interface_NFIX(EFINOC,EFNFIX)
...
!-----------------------------------------------------------------------
     ...
     &                  SNACTM, NODRGM, DWNODI, NDTHMX, CNODCR
        IF (ERR .NE. 0) CALL ERROR(ERRKEY,ERR,FILECC,LNUM)
	PlantN%SNACTM=SNACTM
	PlantN%NODRGM=NODRGM
	PlantN%DWNODI=DWNODI
	PlantN%NDTHMX=NDTHMX
	PlantN%CNODCR=CNODCR
        CALL IGNORE(LUNCRP,LNUM,ISECT,C80)
        ...
        IF (ERR .NE. 0) CALL ERROR(ERRKEY,ERR,FILECC,LNUM)
	PlantN%TYPNGT=TYPNGT 
	PlantN%TYPFXT=TYPFXT
	PlantN%TYPFXD=TYPFXD
	PlantN%TYPFXW=TYPFXW
	PlantN%TYPFXA=TYPFXA
	CALL PUT(PlantN)
      ENDIF
...
!-----------------------------------------------------------------------
	...
	PlantN%CNOD=CNOD
	PlantN%DWNOD=DWNOD
	PlantN%DWNODA=DWNODA
	PlantN%NDTH=NDTH
	PlantN%NFIXN=NFIXN
	PlantN%NODGR=NODGR
	PlantN%WTNFX=WTNFX
	PlantN%SDWNOD=SDWNOD
	PlantN%SENNOD=SENNOD
	CALL PUT(PlantN)
!***********************************************************************
!***********************************************************************
...
!-----------------------------------------------------------------------
	SDWNOD=PlantN%SDWNOD
	DWNODI=PlantN%DWNODI
	DWNODA=PlantN%DWNODA
	PRONOD=PlantN%PRONOD
      IF (SDWNOD .LT. 1) THEN
         DWNOD  = DWNODI * PLTPOP
         SDWNOD = 1
...
!-----------------------------------------------------------------------
	TYPFXT=PlantN%TYPFXT
	TYPNGT=PlantN%TYPNGT
	TYPFXW=PlantN%TYPFXW	
      LAYERFRAC = 0.0
      DSWP = 0.0
      DNOD = 50.0
      DO I = 1,NLAYR
...
!-----------------------------------------------------------------------
  400 TNFIX  = ACSTF / DNOD
      TNGRO  = ACSTG / DNOD
      FLDACT = FLDSUM / DNOD
      TYPFXD=PlantN%TYPFXD
      TYPFXA=PlantN%TYPFXA	

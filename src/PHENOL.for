...
!-----------------------------------------------------------------------
      ...
      DO J = 1, 20
        ...
        FUDAY(J) = 0.
      ENDDO
	!write(*,*)'PHENOL, SEASINIT ,YRPLT=',YRPLT
!	write(*,*)'PHENOL,TO RSTAGES'
...
C-----------------------------------------------------------------------
	CALL GET(PlantP)
	NVEG0=PlantP%NVEG0
	!write(*,*)'PHENOL,RATE,NVEG0=',NVEG0,'DAS=',DAS,interface_ihru
	!write(*,*)'YRPLT=',YRPLT
...
C-----------------------------------------------------------------------
     ...
     &            (MAX(SAT(I) - SW(I),0.0)) / (SAT(I) - DUL(I))
            ENDIF
          ENDIF
	!write(*,*)'I=',I,TSDEP,DTRY,'ST(I)=',ST(I),'DLAYR(I)=',DLAYR(I)	
          TSDEP = TSDEP + DTRY * ST(I)
	
	!write(*,*)'PHENOL, XDEP=', XDEP , TSDEP,ST(I)
...
C-----------------------------------------------------------------------
      DO J = 2,NPHS
        ...
        FT(J) = 0.0
        DO I = 1,TS
	!write(*,*)CTMP(j),'TB(K)=',TB(K),'TO1(K)=',TO1(K)
	!write(*,*)'TO2(K)=',TO2(K),'TM(K)=',TM(K),'TGRO(I)=',TGRO(I)
...
C-----------------------------------------------------------------------
!	write(*,*)'PHENOL,DAS=',DAS,'NR5=',NR5,'PHZACC(10)=',PHZACC(10)
!	write(*,*)'PHTHRS(10)=',PHTHRS(10)
      IF (DAS .GT. NR5) THEN

...
C-----------------------------------------------------------------------
	TYPPGT=PlantP%TYPPGT
	TYPPGN=PlantP%TYPPGN
	LNREF=PlantP%LNREF
...
C-----------------------------------------------------------------------
      ...
      ELSE
        SLW = 0.0099
      ENDIF
	YPGSLW=PlantP%YPGSLW
	XPGSLW=PlantP%XPGSLW
...
C-----------------------------------------------------------------------
	...
	PlantP%PARMAX=PARMAX
	PlantP%PHTMAX=PHTMAX
      CALL IGNORE(LUNCRP,LNUM,ISECT,CHAR)
	...
	PlantP%PGREF=PGREF
      ...
      IF (ERR .NE. 0) CALL ERROR(ERRKEY,ERR,FILECC,LNUM)
	PlantP%XPGSLW=XPGSLW
      CALL IGNORE(LUNCRP,LNUM,ISECT,CHAR)
      READ(CHAR,'(10F6.0)',IOSTAT=ERR) (YPGSLW(II),II = 1,10)
      IF (ERR .NE. 0) CALL ERROR(ERRKEY,ERR,FILECC,LNUM)
	PlantP%YPGSLW=YPGSLW
	CALL PUT(PlantP)
C-----------------------------------------------------------------------
      CLOSE (LUNCRP)
!	write(*,*)'Subroutinen over (PHOTOIP)'

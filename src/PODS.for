...
!***********************************************************************
	...
	CALL GET(PlantP)
	IF(useSWAT)THEN
	CALL interface_PODS(ECONO,WTPSD,SFDUR,SDPDVR,PODUR,THRESH)
	 !ECONO='CO0001'
	!WTPSD
	!SFDUR
	!SDPDVR
...
!-----------------------------------------------------------------------
      ...
      ELSE
        ...
        IF (ERR .NE. 0) CALL ERROR(ERRKEY,ERR,FILECC,LNUM)
	PlantP%PROLFF=PROLFF !
	PlantP%PROSHI=PROSHI!
	PlantP%PROSHF=PROSHF !
      ENDIF
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
	...
	PlantP%POTCAR=POTCAR! 
	PlantP%POTLIP=POTLIP!
	PlantP%AGRSD3=AGRSD3! 
	PlantP%ANINSD=ANINSD!
	PlantP%CUMSIG=CUMSIG! 
	PlantP%RSD=RSD	!
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
	...
	PlantP%AGRSD3=AGRSD3
	PlantP%SDNO=SDNO
	PlantP%PGAVLR=PGAVLR
	PlantP%RPRPUN=RPRPUN
	PlantP%WTSHE=WTSHE
	PlantP%WTSD=WTSD
	PlantP%WTSHMT=WTSHMT
	PlantP%WTSHM=WTSHM
	PlantP%WTABRT=WTABRT
      PlantP%FNINSH=FNINSH
      PlantP%NAVPOD=NAVPOD
      PlantP%NGRSD=NGRSD				
	PlantP%NGRSH=NGRSH
	PlantP%NR2TIM=NR2TIM
	PlantP%PCTMAT=PCTMAT
	PlantP%PGNPOD=PGNPOD
	PlantP%PODNO=PODNO
	PlantP%WSDDTN=WSDDTN
	PlantP%WSHDTN=WSHDTN
...
!-----------------------------------------------------------------------
	CALL GET(PlantP)
	PROSHI=PlantP%PROSHI
	POTCAR=PlantP%POTCAR
	POTLIP=PlantP%POTLIP
        ACCAGE  = 0.0
        AFLW    = 0.0
        CNSTRES = 1.0
        ...
        DO NPP = 1, NCOHORTS
          ...
          AVTEM(NPP) = 0.0
        ENDDO
      	FNINSD=PlantP%FNINSD
	AGRSD3=PlantP%AGRSD3
...
!-----------------------------------------------------------------------
	CNSTRES=PlantP%CNSTRES
	CPSTRES=PlantP%CPSTRES
        CNSTRES = 0.875 * CNSTRES + 0.125 * NSTRES
        CPSTRES = 0.875 * CPSTRES + 0.125 * PStres2

	PlantP%CNSTRES=CNSTRES
	PlantP%CPSTRES=CPSTRES	
...
!-----------------------------------------------------------------------
	XSWFAC=PlantP%XSWFAC
	YSWFAC=PlantP%YSWFAC
	XSWBAR=PlantP%XSWBAR
	YSWBAR=PlantP%YSWBAR
...
!-----------------------------------------------------------------------
	SHLAG=PlantP%SHLAG
	LNGSH=PlantP%LNGSH
	LNGPEG=PlantP%LNGPEG
	!write(*,*)'PODS: YRDOY=',YRDOY,'YRNR2=',YRNR2
	!write(*,*)'_______________TOIMIIKO__________________'
        IF (YRDOY .GT. YRNR2 .AND. YRNR2 .GT. 0) THEN
          DO 2100 NPP = 1, NR2TIM
            ...
            ADDSHL = 0.0
            SUPDAY = 1.0
	!write(*,*)'PAGE=',PAGE,'LNGSH=',LNGSH
            IF (PAGE .LE. LNGSH) THEN
	!write(*,*)'SHELN(NPP)=',SHELN(NPP),'GRRAT1=',GRRAT1	
...
!-----------------------------------------------------------------------
!	write(*,*)'--------PODS------'
!	write(*,*)'WSHDTN=',WSHDTN,'ADDSHL=',ADDSHL
...
!		write(*,*)'WTABRT=',WTABRT,'WTABR=',WTABR
              WTABRT = WTABRT + WTABR
            ENDIF
 2200     ENDDO
!-----------------------------------------------------------------------
...
!	write(*,*)'PMAX=',PMAX,'PGAVLR=',PGAVLR
!	write(*,*)'SDVAR=',SDVAR,'AGRSD1=',AGRSD1,'SDPDVR=',SDPDVR
!	write(*,*)'PODUR=',PODUR
	!if(PMAX==0)STOP
	MNESPM=PlantP%MNESPM
...
!	write(*,*)'PODADD=',PODADD,PMAX,TEMPOD,DRPP
!	write(*,*)'MIN=',SWADD1,SWADD2,RNITPD
!	write(*,*)'MAX=',1-ACCAGE
...
!	write(*,*)'NGRSD=',NGRSD,'NGRSH=',NGRSH
!	write(*,*)'WSDDTN=',WSDDTN,'WSHDTN=',WSHDTN
!	write(*,*)'--------------PODS----------------------------'
!***********************************************************************
!***********************************************************************
!     OUTPUT

...
!      IF (CROP .NE. 'FA' .AND. ISWWAT .NE. 'N') THEN
      	...
      	END IF
	!CumRootMass=0
        ! PlantR%CumRootMass=CumRootMass
	CumRootMass=0.0
!	Call PUT(PlantR)	
!***********************************************************************
!***********************************************************************
...
!-----------------------------------------------------------------------
	CALL GET(PlantR)
	RFAC1=PlantR%RFAC1
	RTDEPI=PlantR%RTDEPI
!	write(*,*)'RTDEPI=',RTDEPI,'RFAC1=',RFAC1	
      CALL INROOT(
     &  DLAYR, FRRT, NLAYR, PLTPOP, RFAC1, RTDEPI, WTNEW, !Input
     &  RLV, RTDEP)                                       !Output
!	write(*,*)'ROOTS:EMERG,RLV=',RLV
      RFAC3 = RFAC1
...
C-----------------------------------------------------------------------
	YRTFAC=PlantR%YRTFAC
	XRTFAC=PlantR%XRTFAC
	RFAC1=plantR%RFAC1
      RFAC2 = TABEX(YRTFAC, XRTFAC, VSTAGE, 4)
      RLNEW = WRDOTN * RFAC1 / 10000.
      CGRRT = AGRRT * WRDOTN
!	write(*,*)'ROOTS:RFAC2=',RFAC2,'RLNEW=',RLNEW,'RFAC1=',RFAC1
C-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
	...
	!write(*,*)'Root:RTSEN=',RTSEN,'DTX=',DTX
	!write(*,*)'DLAYR(L)=',DLAYR(L)	
...
!	write(*,*)'Root:RLSEN(L)',RLSEN(L)*DLAYR(L)/RFAC3*1.E5
!	write(*,*)'Root:RLV_WS',RLV_WS(L)*DLAYR(L)/RFAC3*1.E5
...
!	write(*,*)'ROOTS,root senesce,SRDOT=',SRDOT
!	write(*,*)'ROOOOOOOOOOOOOOOOOOOOOOTTTTTTTTTTTTTTTTTTTTSSSSSSSS'
...
!	write(*,*)'WRDOTN=',WRDOTN,'SRDOT=',SRDOT 
!	write(*,*)'CumRootMass=',CumRootMass,'TotRootMass=',TotRootMass           
!***********************************************************************
!***********************************************************************
...
!	write(*,*)'ROOTS:',FILECC
	...
	PlantR%RTSDF=RTSDF
        ...
        IF (ERR .NE. 0) CALL ERROR(ERRKEY,ERR,FILECC,LNUM)
	PlantR%XRTFAC=XRTFAC
	PlantR%YRTFAC=YRTFAC
        CALL IGNORE(LUNCRP,LNUM,ISECT,CHAR)
        READ(CHAR,'(12X,2F6.0)',IOSTAT=ERR) PORMIN, RTEXF
        IF (ERR .NE. 0) CALL ERROR(ERRKEY,ERR,FILECC,LNUM)
	PlantR%PORMIN=PORMIN
	PlantR%RTEXF=RTEXF
        CALL IGNORE(LUNCRP,LNUM,ISECT,CHAR)
        ...
        IF (ERR .NE. 0 .OR. RWMTXT .NE. 'RTWTMIN') THEN
          RTWTMIN = 0.0
        ENDIF
	PlantR%RTWTMIN=RTWTMIN
	PlantR%RWMTXT=RWMTXT
	CALL PUT(PlantR)
      ENDIF

...
c             If there was an error, write warning to WARNINGS.OUT
              IF (EERROR) THEN
                  CALL WARNING(1,'CULT COEFF', ERRMSG)
                   WRITE(*, '(A)') ERRMSG(1) ! MODIFICATION RRMOWING d from begining of a line
              ENDIF
...
c                 :::::::::::::::::::::::::::::::::::::::
     ...
     &                                          LNUM)
                  ...
                  WRITE(*, '(2A)') 'Species file path is ', FILECC ! MODIFICATION Removing D from the begin on line
...
c                               Detect invalid characters:
     ...
     &                             .LT. 1)
     &                            THEN
                                   WRITE(*,*) 'Invalid! ', K, LINE(K:K) ! MODIFICATION remowind D from beg of line
                                    INVALID = .TRUE.
                                    EXIT
                                  ENDIF
                                ENDDO
...
c                                  TMP_STR = TRIM(LINE(1:EQ_IND))
                                 ...
                                 WRITE(*, '(A, I3)') TMP_STR, EQ_IND ! MODIFICATION removind D of beg of line

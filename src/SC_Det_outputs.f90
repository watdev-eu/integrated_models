...
11      format('season, pltype, year, doy, das, dap, phy, above_ground_internode, above_ground_leaf, leaf_alive,&
 phytomer_attribute_name, phytomer_attribute_units, phytomer_attribute_value')
...
21      format('season, pltype, year,   doy,   das,   dap, gaus_hour, gaus_lai,   lai, frac_li,&
             amax, eff, gaus_hour_abs, gaus_lai_abs,  qleaf_layer,      a_layer, dir_par, dif_par, total_par')
22      format('   dml,    dml, yyyy, 0-366, 0-inf, 0-inf,       0-5,      0-5, m2/m2,     0-1,&
 micromol m-2 s-1,   -,          hour,          lai, micromol m-2 s-1, micromol m-2 s-1,   W m-2,   W m-2,     W m-2')
...
31      format('season, pltype, year,   doy,   das,   dap, watdmd, swfacp, swface, tmn, tempfac_pho, tempfac_per,&
 co2, pho_fac_co2, diacem, agefactor_amax, agefactor_per, sug_it_BG, amaxfbfac,      dtg,          per')
32      format('   dml,    dml, yyyy, 0-366, 0-inf, 0-inf,    0-1,    0-1,    0-1,  oC,         0-1,         0-1,&
 ppm,         0-1, oCdays,            0-1,           0-1,    ton/ha,       0-1,    g m-2,     mm day-1')
        ...
        IF (YRDOY .GE. YRPLT) THEN
            ...
            IF ((MOD(DAS,FROP) .EQ. 0) .OR. (YRDOY .EQ. YRPLT)) THEN     ! Daily output every FROP days on planting date
                ...
                do phy = 1, CaneCrop % n_ph
                    write(NOUTDG_det,113) CaneCrop % seqnow, ',', CaneCrop % pltype, ',', year, ',', doy, ',', das, ',', dap, ',',&
 phy, ',', CaneCrop % fl_it_AG(phy), ',', CaneCrop % fl_lf_AG(phy), ',', CaneCrop % fl_lf_alive(phy), ',',&
 'Leaf Age'                   , ',', 'Cdays'   , ',', CaneCrop % phprof(phy,1)
                    write(NOUTDG_det,113) CaneCrop % seqnow, ',', CaneCrop % pltype, ',', year, ',', doy, ',', das, ',', dap,&
 ',', phy, ',', CaneCrop % fl_it_AG(phy), ',', CaneCrop % fl_lf_AG(phy), ',', CaneCrop % fl_lf_alive(phy), ',',&
 'Leaf Total DW'              , ',', 'g'       , ',', CaneCrop % phprof(phy,6)
                    write(NOUTDG_det,113) CaneCrop % seqnow, ',', CaneCrop % pltype, ',', year, ',', doy, ',', das, ',', dap,&
 ',', phy, ',', CaneCrop % fl_it_AG(phy), ',', CaneCrop % fl_lf_AG(phy), ',', CaneCrop % fl_lf_alive(phy), ',',&
 'Leaf Area'                  , ',', 'cm2'     , ',', CaneCrop % phprof(phy,5)
                    write(NOUTDG_det,113) CaneCrop % seqnow, ',', CaneCrop % pltype, ',', year, ',', doy, ',', das, ',', dap,&
 ',', phy, ',', CaneCrop % fl_it_AG(phy), ',', CaneCrop % fl_lf_AG(phy), ',', CaneCrop % fl_lf_alive(phy), ',',&
 'Internode Age'              , ',', 'Cdays'   , ',', CaneCrop % phprof(phy,58)
                    write(NOUTDG_det,113) CaneCrop % seqnow, ',', CaneCrop % pltype, ',', year, ',', doy, ',', das, ',', dap,&
 ',', phy, ',', CaneCrop % fl_it_AG(phy), ',', CaneCrop % fl_lf_AG(phy), ',', CaneCrop % fl_lf_alive(phy), ',',&
 'Internode Total DW'         , ',', 'g'       , ',', CaneCrop % phprof(phy,50)
                    write(NOUTDG_det,113) CaneCrop % seqnow, ',', CaneCrop % pltype, ',', year, ',', doy, ',', das, ',', dap,&
 ',', phy, ',', CaneCrop % fl_it_AG(phy), ',', CaneCrop % fl_lf_AG(phy), ',', CaneCrop % fl_lf_alive(phy), ',',&
 'Internode Structural DW'    , ',', 'g'       , ',', CaneCrop % phprof(phy,51)
                    write(NOUTDG_det,113) CaneCrop % seqnow, ',', CaneCrop % pltype, ',', year, ',', doy, ',', das, ',', dap,&
 ',', phy, ',', CaneCrop % fl_it_AG(phy), ',', CaneCrop % fl_lf_AG(phy), ',', CaneCrop % fl_lf_alive(phy), ',',&
 'Internode Total Sugars DW'  , ',', 'g'       , ',', CaneCrop % phprof(phy,52)
                    write(NOUTDG_det,113) CaneCrop % seqnow, ',', CaneCrop % pltype, ',', year, ',', doy, ',', das, ',', dap,&
 ',', phy, ',', CaneCrop % fl_it_AG(phy), ',', CaneCrop % fl_lf_AG(phy), ',', CaneCrop % fl_lf_alive(phy), ',',&
 'Internode Sucrose DW'       , ',', 'g'       , ',', CaneCrop % phprof(phy,53)
                    write(NOUTDG_det,113) CaneCrop % seqnow, ',', CaneCrop % pltype, ',', year, ',', doy, ',', das, ',', dap,&
 ',', phy, ',', CaneCrop % fl_it_AG(phy), ',', CaneCrop % fl_lf_AG(phy), ',', CaneCrop % fl_lf_alive(phy), ',',&
 'Internode Hexose DW'        , ',', 'g'       , ',', CaneCrop % phprof(phy,54)
                    write(NOUTDG_det,113) CaneCrop % seqnow, ',', CaneCrop % pltype, ',', year, ',', doy, ',', das, ',', dap,&
 ',', phy, ',', CaneCrop % fl_it_AG(phy), ',', CaneCrop % fl_lf_AG(phy), ',', CaneCrop % fl_lf_alive(phy), ',',&
 'Internode Length'           , ',', 'mm'      , ',', CaneCrop % phprof(phy,16)
                    write(NOUTDG_det,113) CaneCrop % seqnow, ',', CaneCrop % pltype, ',', year, ',', doy, ',', das, ',', dap,&
 ',', phy, ',', CaneCrop % fl_it_AG(phy), ',', CaneCrop % fl_lf_AG(phy), ',', CaneCrop % fl_lf_alive(phy), ',',&
 'Phytomer Age'               , ',', 'Cdays'   , ',', CaneCrop % phprof(phy,12)
                    write(NOUTDG_det,113) CaneCrop % seqnow, ',', CaneCrop % pltype, ',', year, ',', doy, ',', das, ',', dap,&
 ',', phy, ',', CaneCrop % fl_it_AG(phy), ',', CaneCrop % fl_lf_AG(phy), ',', CaneCrop % fl_lf_alive(phy), ',',&
 'Internode Fiber Fraction'   , ',', 'Cdays'   , ',', CaneCrop % phprof(phy,17)
                    write(NOUTDG_det,113) CaneCrop % seqnow, ',', CaneCrop % pltype, ',', year, ',', doy, ',', das, ',', dap,&
 ',', phy, ',', CaneCrop % fl_it_AG(phy), ',', CaneCrop % fl_lf_AG(phy), ',', CaneCrop % fl_lf_alive(phy), ',',&
 'Internode Sugars Fraction'  , ',', 'Cdays'   , ',', CaneCrop % phprof(phy,18)
                    write(NOUTDG_det,113) CaneCrop % seqnow, ',', CaneCrop % pltype, ',', year, ',', doy, ',', das, ',', dap,&
 ',', phy, ',', CaneCrop % fl_it_AG(phy), ',', CaneCrop % fl_lf_AG(phy), ',', CaneCrop % fl_lf_alive(phy), ',',&
 'Internode Sucrose Fraction' , ',', 'Cdays'   , ',', CaneCrop % phprof(phy,19)
                    write(NOUTDG_det,113) CaneCrop % seqnow, ',', CaneCrop % pltype, ',', year, ',', doy, ',', das, ',', dap,&
 ',', phy, ',', CaneCrop % fl_it_AG(phy), ',', CaneCrop % fl_lf_AG(phy), ',', CaneCrop % fl_lf_alive(phy), ',',&
 'Internode Hexose Fraction'  , ',', 'Cdays'   , ',', CaneCrop % phprof(phy,20)
                    write(NOUTDG_det,113) CaneCrop % seqnow, ',', CaneCrop % pltype, ',', year, ',', doy, ',', das, ',', dap,&
 ',', phy, ',', CaneCrop % fl_it_AG(phy), ',', CaneCrop % fl_lf_AG(phy), ',', CaneCrop % fl_lf_alive(phy), ',',&
 'Pfac Struc'                 , ',', '0-1'     , ',', CaneCrop % phprof(phy,15)
                enddo

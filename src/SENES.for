...
!-----------------------------------------------------------------------
     ...
     &      (SENPOR(II),II=1,4),(SENMAX(II),II=1,4)
        IF (ERR .NE. 0) CALL ERROR(ERRKEY,ERR,FILECC,LNUM)
	PlantS%SENRTE=SENRTE
	PlantS%SENRT2=SENRT2
	PlantS%SENDAY=SENDAY
	PlantS%ICMP=ICMP
	PlantS%TCMP=TCMP
	PlantS%XSTAGE=XSTAGE
	PlantS%XSENMX=XSENMX
 	PlantS%SENPOR=SENPOR
	PlantS%SENMAX=SENMAX
	CALL PUT(PlantS)
      ENDIF
...
C-----------------------------------------------------------------------
	...
	XSTAGE=PlantS%XSTAGE
        IF (VSTAGE .GE. 5.0) THEN
          ...
          ENDIF
        ENDIF
	!write(*,*)'Natural senescence prior seed growth'
	!write(*,*)'SENES,SLDOT=',SLDOT
	!write(*,*)'PORLFT=',PORLFT
	!write(*,*)'EHTO',WTLF*(1-RHOL),'<',CLW*PORLFT
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
	...
	!write(*,*)'SENES,by low light SLDOT=',SLDOT
C-----------------------------------------------------------------------
C     Calculate senescence due to water stress.
C-----------------------------------------------------------------------
	SENDAY=PlantS%SENDAY
	SENMAX=PlantS%SENMAX
	...
	!write(*,*)'SENES,due water stress SLDOT=',SLDOT
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
	...
	SENRT2=PlantS%SENRT2
        IF (WTLF .GT. 0.0001) THEN
          SLDOT = WTLF * SENRT2
          SLNDOT = SLDOT
          SSDOT = SLDOT * PORPT
          SSNDOT = SSDOT
!      write(*,*)'SENES,after R7 DAS>NR7,SLDOT=',SLDOT,'WTLF=',WTLF
!	write(*,*)'SENRT2=',SENRT2
	!STOP
        ELSE
          SLDOT = 0.0

...
!MODIFICATION
	IF(useSWAT)THEN
	! Come to inteface
		!write(*,*)'Coming to interface to read SOILPROP'
		CALL interface_SOILPROP
		CALL GET(SOILPROP)
		SLNO=SOILPROP%SLNO
	...
	!	write(*,*)'SOILDYN',DLAYR,'interface_ihru=',interface_ihru
	ELSE		
...
!     Slightly weathered is CEC:CLAY >16 mmol/kg
	!write(*,*)'SOILDYN DO loop, NLAYR',NLAYR
      DO L = 1, NLAYR
	!write(*,*)'SOILDYN, CaCO3=',CaCO3(L)
!       First check for calcareous soil
        IF (CaCO3(L) > 0.15) THEN
          SOILLAYERTYPE(L) = 'CALCAREOUS       '
          CYCLE
        ELSEIF (CaCO3(L) < 0.) THEN
!	write(*,*)'SOILDYN ph=',PH(L),ISWITCH%ISWPHO,MULTI
...
!       Next check for andisol
        IF (VOLCANIC) THEN
          ...
          CYCLE
        ENDIF
!	write(*,*)'SOILDYN, CEC=',CEC(L),'CLAY=',CLAY(L)
!       Last classify by slightly or higly weathered (or unknown)
     ...
     &        '("This may affect results of soil phosphorus model.")')
        ...
        ENDIF
	!write(*,*)'SOILDYN, SOILLAYERTYPE=', SOILLAYERTYPE(L)
      ENDDO
...
!     RGIF_INIT = RGIMPF
      ...
      CRAIN     = 0.
      LCRAIN    = 0.
!	write(*,*)'End of SOILPROP initialization'
!***********************************************************************
!***********************************************************************
...
!     Skip initialization for sequenced runs:
	...
	END IF
      ...
      DLAYR_INIT=DLAYR 	
	BD_INIT=BD
!	write(*,*)'DLAYR_SOM=',DLAYR_SOM,'DLAYR_INIT',DLAYR_INIT
	!STOP
!-----------------------------------------------------------------------
      DO L = 1, NLAYR
!       Conversion from kg/ha to ppm (or mg/l).  Recalculate daily.
...
!         RGIMPF = RGIF_INIT
      ...
      ELSE
	!write(*,*)'SOILDYN HERE',DLAYR_SOM
        BD     = BD_SOM
        DLAYR  = DLAYR_SOM
      ENDIF
	!write(*,*)'SOILDYN,MIDLLE,DLAYR=',DLAYR
      DUL = DUL_SOM
      LL  = LL_SOM
...
!       Available water capacity (mm)
      ...
      SOILPROP % SWCN   = SWCN 
      SOILPROP % POROS  = POROS  
      !write(*,*)'SOILDYN OVER,SOILPROP=',DLAYR	
      CALL PUT(SOILPROP)
...
!     SUBROUTINE OPSOILDYN -- output dynamic soil properties
	...
	!write(*,*)'SOILDYN:interface_ihru=',interface_ihru
!***********************************************************************
!***********************************************************************
!     Seasonal initialization
!***********************************************************************

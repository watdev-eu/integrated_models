...
!	write(*,*)'SoilPi_init:',SOILLAYERTYPE
!     ------------------------------------------------------------------
!     Initialize.
      PiActive   = 0.0
      PiStable   = 0.0
      ...
      IF (ISWPHO == 'N') RETURN
      IF(useSWAT)THEN
	CALL interface_SoilPi_init(PiActive,PiStable,PiLabile,K_LAB2ACT,
     &K_ACT2LAB,K_ACT2STA,K_STA2ACT)
      ELSE
!	write(*,*)'SoilPi_init,ISWPHO=',ISWPHO
!     ------------------------------------------------------------------
!     USE HEDLEY DATA
!     ------------------------------------------------------------------
...
!  *****************************************************************
      ...
      DO L = 1, NLAYR
!	write(*,*)'Soil_Pi_init, L=',L,'useHedley=',UseHedley 
        NMSG = 0 
        IF (.NOT. UseHedley) THEN
          Method = PMethod
          PiLabile(L) = -99.
          ...
          NMSG = 3
!	write(*,*)'SOilPi_init,Pmeasured',PMeasured(L)
...
!           Labile P measured directly with resin extraction or Pi Strip
            ...
            GOTO 1000
          ENDIF  
!	write(*,*)'SoilPi_init. ph ja type:',ph(L),SOILLAYERTYPE(L)
!         Check valid method for pH
     ...
     & "Olsen P extraction method is not valid for pH values below 4.5."
            ...
            ENDIF
          ENDIF		
	!write(*,*)'We are here',SOILLAYERTYPE(L)
...
!	write(*,*)'SoilPi_init. Method=',Method
...
!	write(*,*)'SoilPi_init. Here we are.END',PiLabile
	!END IF	! Enf of SWAT IF
...
!	write(*,*)'SoilPi_init,inside NOT UseHedleys'
        ENDIF       !Not UseHedley

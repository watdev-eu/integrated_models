...
!MODIFICATION JJ 
node(BR,LF)%NPOOLSN = AMAX1 (0.0,((node(BR,LF)%NODEWT * (woodyWeight())/(STWTP+CRWTP))*&
(node(BR,LF)%SANC - node(BR,LF)%SNCM)*NUSEFAC))  
                    ELSE
                        node(BR,LF)%NPOOLSN = 0.0
                    ENDIF
node(BR,LF)%NPOOLLN = AMAX1 (0.0,(((leafAreaLeftToSenesce(node(BR,LF))/LAWL(1)) / (1.0-LPEFR))*&
(node(BR,LF)%LANC-node(BR,LF)%LNCM)*NUSEFAC))
            ...
            IF (node(0,0)%NFLF2 > node(0,0)%AFLF .OR. node(0,0)%NFLF2 > WFG) THEN
                ...
                DO BR = 0, BRSTAGEINT                                                                                        !LPM23MAY2015 To consider different N demand by node according with its age                                                                       
                    DO LF = 1, LNUMSIMSTG(BR)
                        IF (GROSTP>0.0) THEN
node(BR,LF)%SNDEMN = AMAX1(0.0,node(BR,LF)%NODEWTG)*node(BR,LF)%SNCX &
* AMIN1(node(0,0)%AFLF,node(0,0)%NFLF2)
                            SNDEM = SNDEM + node(BR,LF)%SNDEMN
                        ENDIF
                        IF (GROLFP>0.0) THEN 
node(BR,LF)%LNDEMN = (AMAX1(0.0,((node(BR,LF)%LAGLT/PLAGSB2)*GROLFP))*node(BR,LF)%LNCX) &
...
SEEDNUSE2 = AMAX1(0.0,AMIN1(SEEDN-SEEDNUSE,LNDEM+SNDEM+RNDEM-RSNUSED-SEEDNUSE-NUPD,SEEDNI/SDDUR*(TT/STDAY)))  !EQN 205
                ...
                DO BR = 0, BRSTAGEINT                                                                                                                                                               
                    DO LF = 1, LNUMSIMSTG(BR)
                        ...
                        IF (PLAGSB2 > 0.0 .AND. isLeafExpanding(node(BR,LF))) THEN
node(BR,LF)%NDEMLMN = AMAX1(0.0,((node(BR,LF)%LAGLT/PLAGSB2)*GROLFP)) &
* node(BR,LF)%LNCM * AMIN1(node(0,0)%AFLF,node(0,0)%NFLF2)
                        ENDIF  
                    ENDDO
                ENDDO
                ...
                IF (NDEMMN > 0.0) THEN
                    ...
                    DO BR = 0, BRSTAGEINT                                                                                                                                                              
                        DO LF = 1, LNUMSIMSTG(BR)
                            IF (GROSTP > 0.0) THEN
SNUSEN(1,BR,LF) = node(BR,LF)%NODEWTG * AMIN1(node(0,0)%AFLF,node(0,0)%NFLF2)*&
 node(BR,LF)%SNCM * AMIN1(1.0,NULEFT/NDEMMN)
                                SNUSE(1) = SNUSE(1)+ SNUSEN(1,BR,LF)
                            ENDIF
                            IF (GROLFP > 0.0 .AND. isLeafExpanding(node(BR,LF))) THEN
LNUSEN(1,BR,LF) = AMAX1(0.0,((node(BR,LF)%LAGLT/PLAGSB2)*GROLFP)) * &
AMIN1(node(0,0)%AFLF,node(0,0)%NFLF2) * node(BR,LF)%LNCM*AMIN1(1.0,NULEFT/NDEMMN) 
                                LNUSE(1) = LNUSE(1) + LNUSEN(1,BR,LF)
                            ENDIF
                        ENDDO
                    ENDDO

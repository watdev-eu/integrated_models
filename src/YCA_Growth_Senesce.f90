...
!***************************************************************************************************************************
	...
	! MODIFICATION LINE TRUNCATED BY JJ 11.4.2024
                        node(BR,LF)%LAPSTMP = AMIN1((leafAreaLeftToSenesce(node(BR,LF))),(node(BR,LF)%LATL3T*&
(AMIN1((node(BR,LF)%LAGETT+(dailyGrowth())-(LLIFGTT+LLIFATT)), (dailyGrowth()))/LLIFSTT)))         !EQN 372
        ...
        IF (ISWNIT /= 'N') THEN
            ...
            DO BR = 0, BRSTAGEINT                                                                                        !LPM 21MAR15
                DO LF = 1, LNUMSIMSTG(BR)   
!MODIFICATION LINE TRUNCATED by JJ 11.4.2024
                    IF (plantLeafAreaLeftToSenesce() > 0.0 .AND. leafAreaLeftToSenesce(node(BR,LF)) > 0.0 .AND.&
 .NOT. willLeafStillGrowingToday(node(BR,LF))) THEN
                        node(BR,LF)%SENNLFGN = AMAX1(0.0,(LFWT*(node(BR,LF)%LAPSTMP/plantLeafAreaLeftToSenesce()))*LNSC)
	! MODIFICATION BY JJ 11.4.2024
                        node(BR,LF)%SENNLFGRSN = AMAX1(0.0,(LFWT*(node(BR,LF)%LAPSTMP/plantLeafAreaLeftToSenesce()))&
*(node(BR,LF)%LANC-LNSC))                                 !EQN 379

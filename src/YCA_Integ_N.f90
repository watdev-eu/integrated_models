...
node(BR,LF)%LEAFNN = AMAX1(0.0,node(BR,LF)%LEAFNN  + LNUSEN(1,BR,LF)+ LNUSEN(2,BR,LF) - node(BR,LF)%SENNLFGN&
 - node(BR,LF)%SENNLFGRSN - (LNPH/Lcounth) - node(BR,LF)%LEAFNEXCESSN) 

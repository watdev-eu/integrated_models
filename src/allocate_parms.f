...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      use parm
      use smrt_parm, only: mf_active !rtb modflow
	integer::ERROR            
      allocate (alph_e(mhru))
      allocate (co_p(mhru))
!	allocate(foc_m(mhru),STAT=ERROR)!
!	write(*,*)'ERROR',ERROR		      
...
!     changed for jennifer b
      msubo = 22
      mstdo = 130 !rtb added six (sw_gw_q, gw, sw_gw_no3, drn_q, gw_sw_p, sw_gw_p) !rtb drain
      motot = 600             !! (50 years limit)
      
      !Allocate variables necessary for the linkage of SWAT and MODFLOW !tcw
      allocate(rttlc(msub)) !tcw
      allocate(etremain(mhru)) !tcw
      etremain = 0. !tcw
      normOut = .true. !tcw
      
      !Allocate variables necessary for the linkage of SWAT and UZF-RT3D !tcw
      allocate(rchrg_no3_conc(mhru))
      allocate(rchrg_p_conc(mhru)) !rtb P
      allocate(rch_layer(mhru)) !rtb
      rchrg_no3_conc = 0.
      rchrg_p_conc = 0. !rtb P
      rch_layer = 0
...
!    Drainmod tile equations  01/2006
      ...
      allocate (gw_q(mhru))
      allocate (sw_gw_q(mhru)) !rtb 05-25-2016
      allocate (sw_gw_no3(mhru)) !rtb 01-31-2017
      allocate (sw_gw_p(mhru)) !rtb 09-06-2017
      allocate (drn_q(mhru)) !rtb drain 08-08-2017
      allocate (gw_qdeep(mhru))
      allocate (gw_revap(mhru))
      allocate (gw_spyld(mhru))
      allocate (gwht(mhru))
      allocate (vadose_thick_hru(mhru)) !rtb 01-04-2017
      vadose_thick_hru = 100000. !rtb will be calculated by MODFLOW
...
!    Drainmod tile equations  01/2006 
      ...
      allocate (rchrg_p(mhru))    !amount of P getting to the shallow aquifer (rtb modflow)
...
!! Variables for soil P and additional operations mjw
	...
	allocate(rspc_a(mhru))	
      ...
      call zeroini
      call zero_urbn
		! MODIFICATION JJ ADDED TO INTEGRATE DSSAT 19.4.2024
      allocate(es_layer(mlyr))
      allocate(DRN_SWAT(mlyr))
	allocate(soil_texture(mlyr,mhru))		   
      return
      end

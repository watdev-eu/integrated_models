      ...
      do ii = 1, mhyd_bsn
        ...
        select case (icode)
          ...
          case (1)
            call subbasin
            call print_hyd
            !if all subbasin processes have been computed, call MODFLOW if it is active
            if(inum1.eq.msub) then
               if(mf_active==1)call smrt_mf_run !rtb MODFLOW ! MODIFICATION added IF JJ
            endif
          case (2) 
            call route

	...
	INTEGER::NIUNIT ! MODIFICATION JJ
...
C  this file to use the IMPLICIT NONE statement.
	INTEGER::MXPAR,MXCLST,MXINST ! MODIFICATION by JJ
...
C     ------------------------------------------------------------------
     ...
     4                      HDRY,STOPER,CHEDFM,CDDNFM,CBOUFM,VBVL,VBNM
      use mf_rt_link !rtb
	INTEGER::INUNIT,IUDIS,IUZON,IUMLT,MAXUNIT,IGRID,IUOC,IUPVAL ! NODIFICATION by JJ
!	INTEGER::I,IPBUFF,IPHNEW,INBAS !! MODIFICATION by JJ
C
      CHARACTER*(*) VERSION
...
C8E-----READ BOUNDARY ARRAY(IBOUND).
  ...
  280    CONTINUE
      ...
      !Record the number of constant-head cells, for use by RT3D, rtb
      maxcnh = 0
      do K=1,NLAY
        do I=1,NROW
          do J=1,NCOL
...
C6E4----Lookup the operand in the list of existing multipliers
             ...
             CALL UPCASE_SWAT(CTMP2)
             DO 50 MM=1,M-1
               CTMP1=MLTNAM(MM)
               CALL UPCASE_SWAT(CTMP1)
...
C8A-----CHECK FOR DUPLICATE PARAMETER NAME FOR ALL BUT THE FIRST PARAMETER.
        IF (I.GT.1) THEN
          PNI=PARNAM(I)
          CALL UPCASE_SWAT(PNI)
          IM1 = I-1
          DO 60 J=1,IM1
            PNJ=PARNAM(J)
            CALL UPCASE_SWAT(PNJ)
            IF (PNI.EQ.PNJ) THEN

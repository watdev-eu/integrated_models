...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      use parm
      use smrt_parm !SWAT-MODFLOW parameters rtb

      integer :: j, j1, nn, k, sb, isp
      integer :: jj, recharge_layer, dum !rtb
      LOGICAL::EHTO1,EHTO1a,EHTO1b,EHTO1c		
	...
	if (aird(j)>0) then
      ...
      !rtb SWAT-MODFLOW (02-16-2017) Add irrigation water from aquifer (from MODFLOW WELL package cells)
      if(mf_active.eq.1 .and. mf_irrigation.eq.1 .and.
     &                        mf_irrigation_swat.eq.0) then
        
        !determine if sub-basin receives irrigation water from pumped wells; if so, then specify the depth
        if(mf_sub_irrig_flag(inum1).eq.1) then
          
          !only proceed if the HRU is designated as receiving irrigation water from MODFLOW
          !(specified in "swatmf_irrigate.txt" file)
          if(mf_hru_irrig_flag(j).eq.1) then
          
          !depth of irrigation water and depth of surface runoff
          aird(j) = mf_sub_irrig_depth(inum1)*(1-mf_runoff_ratio(inum1)) !mm of irrigation water
          qird(j) = mf_sub_irrig_depth(inum1)*mf_runoff_ratio(inum1)

          !determine volume of water applied (area * depth); sum up to daily total
          mf_hru_irrig = mf_hru_irrig + 
     &                   ((aird(j)+qird(j))/1000)*
     &                   (hru_km(j)*1000000.)
          
          !summary calculations (same as in "irrigate.f" subroutine)
...
!!      aird(j) = 0.
      ...
      profile_z = 0. !rtb
      rch_layer(j) = sol_nly(j) !rtb (set default to lowest layer)
	DRN_SWAT=0.0
      do j1 = 1, sol_nly(j)
...
!      if(j1.EQ.i_sep(j).AND. sol_tmp(j1,j).GT.0.0 .AND. isep_opt(j) .NE. 0) then
	EHTO1a=j1.EQ.i_sep(j)
	EHTO1b=sol_tmp(j1,j).GT.0.0
	EHTO1c=isep_opt(j).NE.0.0
	EHTO1=EHTO1a .AND. EHTO1b .AND.EHTO1c
	IF(EHTO1) THEN	
     ...
     &                    + sptno2concs(isp))  
       ...
       !! determine gravity drained water in layer
        ...
        if (j1 == sol_nly(j)) then
          ...
          if(sepbtm(j).gt.0) then
            dum = 10
          endif
        endif
        latq(j) = latq(j) + latlyr !add to array
        qtile = qtile + lyrtile !add to array
        flat(j1,j) = latlyr + lyrtile !add to array
        sol_prk(j1,j) = sol_prk(j1,j) + sepday
	  if (latq(j) < 1.e-6) latq(j) = 0.
        if (qtile < 1.e-6) qtile = 0.
        if (flat(j1,j) < 1.e-6) flat(j1,j) = 0.
	DRN_SWAT(j1)=sepday+DRN_SWAT(j1)
      end do !go to the next soil layer
      
	DRN_SWAT=DRN_SWAT*0.10
      ...
      !only proceed if soil temperature of second layer is above freezing (> 0 C)
...
!       Updated water table depth D.Moriasi 4/8/2014
        ...
        !now that water table depth has been determined, compute tile drainage flow
        if (ddrain(j) > 0.) then
          
          !if water table is below the drains, then no tile drainage flow is calculated
          if (wt_shall <= d) then
            qtile = 0.
          else

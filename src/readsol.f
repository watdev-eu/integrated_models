...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      character (len=80) :: titldum,soil_layer
...
!      write(*,*)'readsol'
      ...
      read (107,5500) soil_layer
      read (107,5000) (sol_z(j,ihru), j = 1, mlyr)
	if(ihru==1)write(*,*)'soil_layer:',soil_layer      
      apu=index(soil_layer,':')
      apu1=index(soil_layer,'-')	
      !write(*,*)'SOIL_LAYER:',adjustl(soil_layer(apu+1:apu1-1))
...
!      sol_z(j,ihru) = sol_z(j,ihru) / 5.0
      ...
      nly = sol_nly(ihru)
	!Modification
!	write(*,*)'EKA:',soil_layer(apu+1:apu1-1)
      soil_texture(1,ihru)=adjustl(soil_layer(apu+1:apu1-1))
	!write(*,*)'soil_texture:',soil_texture(1,ihru)
	IF(nly.GT.2)THEN
	do j=2,nly-1
		apu2=index(soil_layer(apu1+1:80),'-')
		!write(*,*)'Kerros=',j,soil_layer(apu1+1:apu1+apu2-1)
		soil_texture(j,ihru)=adjustl(soil_layer(apu1+1:apu1+apu2-1))
		apu1=apu1+apu2
	end do
		   apu2=index(soil_layer(apu1+1:80),' ')
!		write(*,*)'LOPPU:',soil_layer(apu1+1:80)
		soil_texture(nly,ihru)=adjustl(soil_layer(apu1+1:apu1+apu2-1))

	ELSE
		IF(nly.EQ.2)THEN
		apu2=index(soil_layer(apu1+1:80),' ')
		soil_texture(2,ihru)=adjustl(soil_layer(apu1+1:apu1+apu2))
		END IF	
	END IF

      if(ihru==1)write(*,*)'SOIL TEXTURE:',soil_texture(1:nly,ihru)
      if(ihru==1)write(*,*)'soil_z:',sol_z(:,ihru)
      eof = 0
      do

...
!         endif
	...
	!write(*,*)'swu:strsw=',strsw(j),'ep_day=',ep_day,'ihru=',j
	if(j==40)THEN
		write(*,*)'wuse=',wuse,'day=',iida,'j=',j
	!if(wuse(1)>0)STOP
	END IF	
      end if
	CALL GET(Land_C)
	...
	!if(j==28)THEN
!		write(*,*)'swu:zz=',zz
!		write(*,*)'DS=',SOILPROP%DS
!		write(*,*)'sol_z=',sol_z(:,j)
!	END IF
	iii=1
	DO ii=1,sol_nly(j)
	  !if(j==28)write(*,*)sol_z(ii,j)/10,zz1(iii)

		IF(sol_z(ii,j)/10<=zz1(iii))THEN
		!if(j==28)write(*,*)'inside ',sol_z(ii,j)/10,zz1(iii)
			Land_C%RWU(iii)=wuse(ii)*10
			iii=iii+1
		ELSE
		iii=iii+1
		END IF
	...
	CALL PUT(LAND_C)
	CALL PUT(SPAM_1)
	!write(*,*)'swu,ep_day=',ep_day
	!if(j==28)WRITE(*,*)'land_c%rwu=',LAND_C%RWU
      return
      end
